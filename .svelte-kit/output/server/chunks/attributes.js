...
const formatStarCount = (starCount) => {
  if (starCount < 1e3) return starCount;
...
const formatSize = (sizeKb) => {
  if (sizeKb < 1e3) return `${sizeKb} KB`;
...
const formatTimeAgo = (inputDate) => {
  const seconds = Math.floor(((/* @__PURE__ */ new Date()).getTime() - new Date(inputDate).getTime()) / 1e3);
...
export {
  formatDescription as a,
  formatStarCount as b,
  convertGhResponse as c,
  findEmoji as d,
  formatLargeNumber as e,
  formatTitle as f,
  formatSize as g,
  formatDate as h,
  formatTimeAgo as i
